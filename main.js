...
function App() {
  ...
  const hdlUpdate = (id, num) => {
    const newState = counters.map( el => ({ ...el, number: (el.id===id && el.number+num >=0) ? el.number + num : el.number })
    )
  ...
  return (
    <><br></br>
      ...
      <h1 className="text-center">SNRU Mr.Piyarat Sopa</h1><br></br>
      <button className="text-center" onClick={hdlAddCounter}>Add Counter</button><br></br>
      <SumInfo total={total}/>
